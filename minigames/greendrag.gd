...
func _on_KinematicBody2D_mouse_entered():
	...
	if not( get_node("../..").bluedrag or get_node("../..").reddrag or get_node("../..").yellowdrag):
		dragReady = true

func _on_KinematicBody2D_mouse_exited():
	dragReady = false
	get_node("../..").greendrag = false

func _on_green_area_body_entered(body):
	# This makes the green dot drop when in correct area
	if Input.is_action_pressed("click") and body.name == "greendrag":
		completed = true
		dragReady = false
